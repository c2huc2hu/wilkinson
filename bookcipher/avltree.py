...
class TokenNode(AVLNode):
    ...
    def find_neighbours(self, k):
        """
        find the nodes on either side of k.
        returns a 2 tuple (lower_bound, upper_bound), or None if they are past the smallest/largest element in the tree.
        lower_bound may equal upper_bound.
        """
        if k == self.key:
            return self, self # found an exact match. return it
        elif k < self.key:
            if self.left is None:
                return None, self
            else:
...
class TokenAVL(AVL):
    '''avl tree with extra data about the token and supports the find_neighbours operation'''
    def insert(self, token):
        '''insert a token'''

        node = TokenNode(None, token)
        if self.root is None:
            # The root's parent is None.
            self.root = node
        else:
    ...
    def delete(self, k):
        ...
        # need to override the call to AVLNode if you want to use this.
        # should be straightforward, but not needed for my code