...
class GPTBeam():
    ...
    def __init__(self, prediction=[], lm_prob=0, lattice_prob=0, last_word_prob=0, lattice_idx=0, exhausted=False):
        ...
        self.last_word_prob = 0 # lattice probability of the last word. needs to be stored for GPT
        self.exhausted = exhausted

    def log_prob(self):
        return self.lm_prob + self.lattice_prob

    def __str__(self):
        if GPTBeam.tokenizer is None:
            return "{}: {} + {} = {}".format(self.prediction, self.lm_prob, self.lattice_prob, self.log_prob, self.lattice_idx)
        else:
            return "{}: {} + {} = {}".format(GPTBeam.tokenizer.decode(self.prediction), self.lm_prob, self.lattice_prob, self.log_prob, self.lattice_idx)
...
def beam_search(source, lm, lattice, beam_width=8, alpha=1):
    ...
    lm.initialize(source)
    beams = [GPTBeam(lm.tokenizer.encode(lm.tokenizer.bos_token))] # list of tokens
    ...
    for wordpiece, wordpiece_idx in lm.tokenizer.encoder.items():
        ...
        if any(ch in wordpiece for ch in ('!"#$%&()*+,-/:;<=>?@[\\]^_`{|}~\t\n ')) or wordpiece.isspace():
            continue
    ...
    while any(not beam.exhausted for beam in beams):
        ...
        new_beams = []
        for beam in beams:
            # If beam is exhausted, we're done
            if beam.exhausted:
                continue
            ...
            # If the token is a literal, add it and inflected forms to the beam directly
                ...
                new_beam = GPTBeam(beam.prediction + lm.tokenizer.encode(source[lattice_idx].plaintext), beam.lm_prob, beam.lattice_prob + 0, lattice_idx)
            ...
            # Otherwise consider new words that can extend the beam
                ...
                for word, word_idx in words.items():
                    ...
                    lattice_prob = lattice.integrate_prob(word, source[lattice_idx])
            ...
            # Case 2: Add an affix. This is always a possibility
            lattice_idx = beam.lattice_idx

            # Add all affixes that are still compatible with the source token
            for suffix, suffix_idx in suffixes.items():
                lm_prob = lm_probs[suffix_idx]
                lattice_prob = lattice.integrate_prob(suffix, source[lattice_idx])

                if lattice_prob > -np.inf:
                    new_beams.append(GPTBeam(beam.prediction + [suffix_idx], beam.lm_prob + alpha*lm_prob, beam.lattice_prob + lattice_prob, beam.last_word_prob + lattice_prob, lattice_idx))
        ...
        beams = list(reversed(sorted(new_beams, key=GPTBeam.log_prob)[:-beam_width]))

        # Print the best beams
        print('Beams (worst to best):\n')
        for i in range(min(beam_width, 10, len(beams))):
            print('Beam # ', i)
            print(beams[i].prediction, beams[i])
        
    return [lm.tokenizer.decode(beam.prediction) for score, beam in exhausted_beams]
