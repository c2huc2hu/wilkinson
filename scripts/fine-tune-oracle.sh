...
corpus='oracle'
train_file='decoding2.txt'
block_size=256

# WILKINSON
# declare -a epochs=(1 3 10 20)
# corpus='wilkinson'
# train_file='wilkinson_letters.txt'u
# block_size=512
...
do
    ...
    python3.7 ~/transformers/examples/language-modeling/run_language_modeling.py \
        --output_dir=language-models/$corpus/epoch-$epoch --model_name_or_path=language-models/$corpus/epoch-$epoch \
        --do_eval --eval_data_file=language-models/oracle/decoding.txt --overwrite_output_dir --block_size=$block_size 2>&1 |
        tee language-models/$corpus/epoch-$epoch/eval-oracle.out

    python3 bookcipher/runtime.py -b 1 --lm language-models/$corpus/epoch-$epoch 2>&1 | tee language-models/$corpus/epoch-$epoch/decoding.out
done
